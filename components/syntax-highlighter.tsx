...
export function PythonSyntaxHighlighter({ code, className = "" }: SyntaxHighlighterProps) {
    ...
    return lines.map((line, lineIndex) => {
      ...
      filteredMatches.forEach((match, index) => {
        ...
        if (match.start > currentIndex) {
          tokens.push(
            <span key={`text-${lineIndex}-${index}`} className="text-foreground">
      ...
      return (
        <div key={lineIndex} className="leading-relaxed">
          {tokens.length > 0 ? tokens : <span className="text-foreground">{line}</span>}
        </div>
      )
    })
  }

  return <div className={`font-mono text-sm ${className}`}>{highlightPython(code)}</div>
}
